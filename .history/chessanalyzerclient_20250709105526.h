...
inline void ChessAnalyzerClient::onReplyFinished()
{
    ...
    if (reply->error() == QNetworkReply::NoError && statusCode < 400) {
        ...
        if (err.error != QJsonParseError::NoError) {
            ...
            if (doc.isArray()) {
                // Empaquetamos el array bajo la clave "array" para preservar la firma existente.
                QJsonObject obj; obj.insert("array", doc.array());
                emit requestSucceeded(endpoint, obj);
            } else {
