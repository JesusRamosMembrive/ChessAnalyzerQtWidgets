...
void PlayerPendingCardWidget::updateInfo(const PlayerInfo &info)
{
    applyProgress(info.progress, info.doneGames, info.totalGames);
    // actualiza status si cambia
    m_statusLabel->setText(info.status);
    const QString startStr = info.requestedAt.isValid() ? info.requestedAt.date().toString("d/M/yyyy") : "-";
    m_startDateLabel->setText("📅 Started: " + startStr);
    m_gamesLabel->setText(QString("Games: %1    Done: %2").arg(info.totalGames).arg(info.doneGames));
}
